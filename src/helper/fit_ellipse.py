...
from src.helper.datatypes import EllipseParameters

def fit_ellipse_to_power_spectra(
        power_spectra: np.ndarray,
        contour_scale: float = 50
):
    power_spectra_norm = power_spectra / np.max(power_spectra)
    contours = measure.find_contours(power_spectra_norm, contour_scale * np.min(power_spectra_norm))
    ...
    ellipse_cartesian_coefficient = fit_ellipse(longest_contour[:, 1], longest_contour[:, 0])
...
def ellipse_cartesian_to_polar(cartesian_coefficients: np.ndarray) -> EllipseParameters:
    ...
    minor_axis = np.sqrt(num / (den * (-fac - a - a)))
